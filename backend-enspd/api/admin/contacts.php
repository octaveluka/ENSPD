<?php
/**
 * Admin : gestion des messages de contact (ENSPD).
 *  GET  ?handled=0|1&q=        -> liste filtrée
 *  POST (CSRF requis), champ "op" :
 *   - op="handle" {id, handled} -> marque traité / non traité
 *   - op="delete" {id}          -> supprime définitivement (RGPD)
 */
...
if ($method === 'POST') {
    ...
    $data = read_json_body();
    $op   = clean_str($data['op'] ?? 'handle', 20);
    $id   = (int) ($data['id'] ?? 0);
    ...
    if ($op === 'delete') {
        ...
        audit_log($admin['id'], 'contact_delete', 'contact', (string) $id);
        json_response(['ok' => true, 'message' => 'Message supprimé.']);
    }
